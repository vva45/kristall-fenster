...
import { categoriesOrdered, de, manufacturersFor, systemByPath } from "../../../../../../lib/catalog";
...
export default async function SystemDetail({ params }: { params: Promise<{ slug: string; manufacturer: string; system: string }> }) {
  ...
  return <main className="mx-auto w-full max-w-[1200px] flex-1 px-5 py-10 md:px-8 md:py-16">
    <nav className="font-mono text-xs text-kamika-steel"><Link href="/catalog">Katalog</Link> <span aria-hidden> / </span><Link href={`/catalog/${category.slug}?brand=${encodeURIComponent(entry.manufacturer.name)}`}>{de(category.name)}</Link></nav>
    <div className="mt-8 grid gap-10 lg:grid-cols-[1.1fr_.9fr]">
      ...
      <article><p className="kamika-eyebrow">{entry.manufacturer.name} · {de(category.name)}</p><h1 className="mt-2 text-4xl md:text-5xl">{entry.system.name}</h1><p className="mt-4 text-lg text-kamika-ink/65"><L t={entry.system.tagline} /></p>{entry.system.description && <p className="mt-5 text-sm text-kamika-ink/70"><L t={entry.system.description} /></p>}
        {entry.system.specs?.length ? <dl className="mt-7 divide-y divide-kamika-mist border-y border-kamika-mist">{entry.system.specs.map((spec, index) => <div key={index} className="flex justify-between gap-6 py-3 text-sm"><dt className="text-kamika-ink/55"><L t={spec.label} /></dt><dd className="text-right font-mono"><L t={localized(spec.value)} />{spec.unit ? ` ${spec.unit}` : ""}</dd></div>)}</dl> : <p className="mt-7 rounded-kamika bg-kamika-blue-50 p-4 text-sm">Technische Daten sind beim Lieferanten angefragt.</p>}
