...
export default async function CategoryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="flex-1">
    <LocalizedMetadata title={category.name} description={category.intro} />
    ...
    <div className="mx-auto max-w-[1440px] px-5 pb-20 md:px-8">
      {category.comingSoon ? <p className="mt-10 rounded-kamika border border-kamika-mist bg-kamika-blue-50 px-4 py-3 text-sm text-kamika-ink/75"><L t={CS.comingSoonLong} /></p> : <>
        {(manufacturers.length > 0 || collections.length > 0) && <nav className="mt-8 flex flex-wrap gap-2" aria-label="Brands and collections / Marken und Kollektionen / Marki i kolekcje">
